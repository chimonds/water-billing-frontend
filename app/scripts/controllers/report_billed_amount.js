...
app.controller('ReportBilledAmountCtrl', function($scope, $http, appService, $cookieStore, $state, $mdDialog, $mdToast, $animate, $rootScope) {
  ...
  $scope.generate = function(form) {
    ...
    appService.getBilledAmountReport(params).success(function(response) {
      $scope.progress = false;
      $scope.error = false;
  ...
  $scope.generateCsv = function() {
    ...
    angular.forEach(accounts, function(value) {
      ...
      $scope.csvData.push({
        ...
        h: value.units,
        i: value.meterRent,
        j: value.otherCharges,
        k: value.amountBilled,
        l: (value.meterRent+ value.otherCharges+ value.amountBilled)
      });
    });
