...
app.controller('SmsCtrl', function($scope, $http, appService, $cookieStore, $state, $mdDialog, $mdToast, $animate, $rootScope) {
  ...
  $scope.editMessageDialog = function(index) {
    $scope.message = $scope.messages[index];

    $mdDialog.show({
      controller: EditMessageDialogController,
      templateUrl: 'views/template/sms_update.html',
      resolve: {
        message: function() {
  ...
  function EditMessageDialogController($scope, $mdDialog, $rootScope, message, appService) {
    var config = appService.getCofig();
    ...
    $scope.update = function(form) {
      ...
      if (myForm.$invalid === false) {
        ...
        var smsId=message.smsId;


        //send request
        appService.updateMessage(request,smsId).success(function(response) {
          $scope.errorOccured = false;
          $scope.errorClass = config.cssAlertSucess;
          $scope.errorMsg = response.message;
          //notify roles page to reload data
