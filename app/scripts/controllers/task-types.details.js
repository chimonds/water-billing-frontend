 ...
 * @ngdoc function
 * @name majiApp.controller:TaskTypesDetailCtrl
 * @description
 * # UsersCtrl
 * Controller of the TaskTypesDetailCtrl
 */

app.controller('TaskTypesDetailCtrl', function ($scope, $http, appService, $cookieStore, $state, $mdDialog, $mdToast, $animate, $rootScope, $stateParams) {
    $scope.taskTypeId = $stateParams.taskTypeId;

    var request = {};
    var taskTypeId = $scope.taskTypeId;

    $scope.$on('onReloadAccountData', function (event) {
        $scope.getTaskTypeProfile();
    });

    $scope.getTaskTypeProfile = function () {
        //send request
        appService.getTaskTypeById(taskTypeId).success(function (response) {
            $scope.taskType = response.payload;
